...
export default async function AdminOutreach({
  ...
  async function handleLogin(formData: FormData) {
    ...
    if (!process.env.ADMIN_USER_ID || !process.env.ADMIN_USER_PASSWORD) {
      redirect(`/admin/outreach/?secret=${secret}&error=server`);
    }
    ...
    if (id === process.env.ADMIN_USER_ID && password === process.env.ADMIN_USER_PASSWORD) {
      ...
      redirect(`/admin/outreach/?secret=${secret}`);
    } else {
      redirect(`/admin/outreach/?secret=${secret}&error=1`);
    }
  }
        ...
        <div className="flex sm:flex-row flex-col gap-6 sm:items-center justify-between mb-6 pb-4 border-b border-border/50">
          ...
          <div className="flex items-center gap-4">
            <div className="px-4 py-2 bg-primary/10 border border-primary/20 rounded-lg hidden sm:block">
              <span className="text-xs font-black text-primary uppercase tracking-widest">Sent Today: {sentToday || 0}/300</span>
            </div>
            <a href={`/admin/leads/?secret=${params.secret}`} className="text-xs font-bold bg-card hover:bg-muted text-foreground px-5 py-2.5 rounded-lg transition-all border border-border flex items-center gap-2">
